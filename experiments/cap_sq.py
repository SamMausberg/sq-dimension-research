...
TAU = INNER / (24 * P)
BUDGET = math.ceil(
    8 * RHO * (math.log(1024) + math.log(8 / float(INNER)) + 1 + math.log(2 / float(INNER)))
)


def family(N, seed):
    pts = np.array(
        [(x, y) for x in range(1, N + 1) for y in range(1, 2 * N * N + 1)], dtype=np.int64
    )
    lin = pts.copy()
    inc = lin[:, 0, None] * pts[None, :, 0] + lin[:, 1, None] == pts[None, :, 1]
    template = np.where(inc, -1, 1).astype(np.int8)
    rng = np.random.default_rng(seed)
    A = template.copy()
    A[inc & (rng.integers(2, size=inc.shape) == 1)] = 1
    ...
    b = lin[:, 1] / (2 * N * N)
    V = np.stack(
        [a * a, -2 * a, np.ones(len(lin)), 2 * a * b, -2 * b, b * b - 1 / (8 * N**4)], axis=1
    )
...
def radial_transform(V):
    ...
    def fg(t):
        t = t - np.mean(t)
        shift = float(np.max(t))
        weights = np.exp(t - shift)
        S = Y.T @ (weights[:, None] * Y)
        sign, ld = np.linalg.slogdet(S)
        if sign <= 0:
            return 1e200, np.zeros(k)
        inv = np.linalg.inv(S)
    ...
    res = minimize(
        ...
        jac=True,
        method="L-BFGS-B",
        options={"maxiter": 1500, "ftol": 1e-13, "gtol": 1e-9, "maxls": 40},
    )
    t = res.x - res.x.max()
    S = Y.T @ (np.exp(t)[:, None] * Y)
    eig, Q = np.linalg.eigh(S)
    if np.min(eig) <= 0:
        raise RuntimeError("No SPD covariance")
...
class CapLaw:
    ...
    def build(self, survivors):
        ...
        tic = time.monotonic()
        ids = np.flatnonzero(survivors)
        k = len(ids)
        T, resid, nit, msg = radial_transform(self.V0[ids])
        deff = T.shape[1] if isinstance(T, np.ndarray) else T.cols
        self.max_resid = max(self.max_resid, resid)
        # For rectangular T, invert its action on the common row span.
        if not isinstance(T, np.ndarray):
            Z, Xp = high_precision.apply(T, self.V0[ids], self.U0)
            self.fallbacks += 1
        else:
            if T.shape[0] == T.shape[1]:
                Xp = self.U0 @ np.linalg.inv(T).T
            else:
                Xp = self.U0 @ np.linalg.pinv(T).T
            Z = self.V0[ids] @ T
            Z /= np.linalg.norm(Z, axis=1)[:, None]
            norms = np.linalg.norm(Xp, axis=1)
            if np.min(norms) == 0:
                raise RuntimeError("A projected point is zero")
            Xp = Xp / norms[:, None]
        state_seed = int.from_bytes(
            hashlib.sha256(str(self.seed).encode() + key).digest()[:8], "little"
        )
        state_rng = np.random.default_rng(state_seed)
        directions = np.vstack([Xp, state_rng.normal(size=(32, deff))])
        directions /= np.linalg.norm(directions, axis=1)[:, None]
        alpha = 1 / math.sqrt(2 * deff)
            ...
            if sign == 1:
                ...
                if len(rid) == 1:
                    for c in [-1, 1]:
                        Sc = S & (self.A[rid[0]] == c)
                        if Sc.any():
                            rectangles.append((Sc, rid.copy(), c))
                elif int(S.sum()) == 1:
                    x = int(np.flatnonzero(S)[0])
                    for c in [-1, 1]:
                        rr = rid[self.A[rid, x] == c]
                        if len(rr):
                            rectangles.append((S.copy(), rr, c))
                else:
                    bad += 1
        if not rectangles:
            raise RuntimeError("No cap rectangles")
        ...
        SM = np.stack([s for s, _, _ in rectangles])
        mass = np.array([len(r) for _, r, _ in rectangles], dtype=np.int64)
        for s, rr, c in rectangles:
            assert np.all(self.A[np.ix_(rr, np.flatnonzero(s))] == c)
        numer = SM.T.astype(np.int64) @ mass
        den = len(rectangles) * k
        if int(numer.min()) * RHO < den:
            raise RuntimeError(
                f"Finite cap law failed exact pointwise coverage: {numer.min()}/{den}, residual {resid}"
            )
        cid = len(self.certs)
        meta = {
            "id": cid,
            "survivors": ids.tolist(),
            "covariance_residual_float": resid,
            "optimizer_iterations": nit,
            "optimizer_message": msg,
            "dimension": deff,
            "discarded_numeric_rectangles": bad,
            "minimum_coverage_numerator": int(numer.min()),
            "coverage_denominator": den,
            "rectangle_count": len(rectangles),
        }
        packed = []
        for ss, rr, cc in rectangles:
            tt = np.zeros(self.A.shape[0], bool)
            tt[rr] = True
            packed.append(
                {
                    "S": base64.b85encode(np.packbits(ss).tobytes()).decode(),
                    "T": base64.b85encode(np.packbits(tt).tobytes()).decode(),
                    "color": cc,
                }
            )
        if self.certstream is not None:
            self.certstream.write(
                json.dumps(dict(meta, rectangles=packed), separators=(",", ":")) + "\n"
            )
            self.certstream.flush()
        self.certs.append(meta)
        val = (rectangles, SM, cid)
        self.cache[key] = val
    ...
    def sample(self, U, V, rng):
        ...
        if not len(live):
            raise RuntimeError("Coverage contradicts empty live catalog")
        j = int(live[int(rng.integers(len(live)))])
        s, rr, c = rect[j]
        tb = np.zeros(len(V), bool)
        tb[rr] = True
            ...
            json.dumps(
                {
                    ...
                    "max_residual": self.max_resid,
                    "seconds": self.seconds,
                    "high_precision_fallbacks": self.fallbacks,
                },
                separators=(",", ":"),
            )
        )
...
class Oracle:
    ...
    def mass(self, S):
        return Fq(int(self.w[S].sum()), self.den)

    def answer(self, S, kind, score=None):
        val = self.mass(S)
        choices = [val] if self.mode == "exact" else [val - TAU, val + TAU]
        if score is None:
            ans = choices[-1]
        else:
            ans = max(choices, key=lambda z: (score(z), z))
        self.count += 1
        self.trace.append(
            {
                "query": self.count,
                ...
                "truth": str(val),
                "answer": str(ans),
                "endpoint": 0 if ans == val else (-1 if ans < val else 1),
                "mask": base64.b85encode(np.packbits(S).tobytes()).decode(),
            }
        )
...
def proper_finish(A, target, S, T, O, eps, events):
    ...
    if len(xs) == 1:
        ...
        ans = O.answer(
            A[j] != h,
            "candidate",
            lambda z: (float(loss) if z <= eps / 2 else 0.0, int(z > eps / 2)),
        )
        if ans <= eps / 2:
            return A[j].copy(), "rectangle_candidate"
        assert j != target
        pool = xs.copy()
        neg = S & (h == -1)
        w = O.mass(neg)
    ...
    for j in candidates[:J]:
        ...
        if ans <= eps / 2:
            return A[j].copy(), "small_tail_candidate"
...
def learn(A, sampler, target, weights, seed, mode="endpoint", proper=False, keep=True):
    h = A[target]
    U = np.ones(A.shape[1], bool)
    V = np.ones(A.shape[0], bool)
    g = np.ones(A.shape[1], dtype=np.int8)
    rng = np.random.default_rng(seed)
    qtrace = []
    events = []
    O = Oracle(weights, h, mode, qtrace)
    rounds = peels = elims = rejects = 0
    reason = "budget"
    rho = Fq(RHO)
    r = 1 / rho
    ...
    if proper:
        ...
        if reason != "budget":
            assert error <= EPS / 2 + TAU
    elif reason != "budget":
        assert error <= INNER / 2
    return {
        "target": int(target),
        "seed": int(seed),
        "oracle": mode,
        "proper": proper,
        "support": np.flatnonzero(weights).tolist(),
        "weights": np.asarray(weights)[np.asarray(weights) > 0].tolist(),
        "denominator": int(np.sum(weights)),
        ...
        "rejects": rejects,
        "termination": reason,
        "prediction": base64.b85encode(np.packbits(g == 1).tobytes()).decode(),
        "queries_trace": qtrace if keep else [],
        "events": events if keep else [],
    }


def make_dist(X, supp, rng, den=10**12):
    vals = rng.dirichlet(np.full(len(supp), 0.5))
    ws = np.maximum(1, np.floor(vals * den).astype(np.int64))
    ws[np.argmax(ws)] += den - int(ws.sum())
    out = np.zeros(X, dtype=np.int64)
    out[supp] = ws
    return out
...
def run(args):
    ...
    for N in args.N:
        ...
        pts, lin, template, A, U0, V0 = family(N, seed + 10 * N)
        X = len(pts)
        sampler = CapLaw(A, U0, V0, seed + 20 * N)
        sampler.certstream = gzip.open(OUT / f"cap_catalogs_N{N}.jsonl.gz", "wt")
        np.savez_compressed(
            OUT / f"family_N{N}.npz", points=pts, lines=lin, template=template, A=A, U0=U0, V0=V0
        )
        eligible = np.flatnonzero((A == -1).sum(1) >= min(2, N))
        if not len(eligible):
            eligible = np.flatnonzero((A == -1).sum(1) > 0)
        cases = []
        # Structured attacks and prior-collapse stresses.
        for j in rng.choice(eligible, min(3, len(eligible)), replace=False):
            incident = np.flatnonzero(template[j] == -1)
            neg = np.flatnonzero(A[j] == -1)
            for supp, label in [
                (incident, "template_line"),
                (neg, "negative_only"),
                (rng.choice(X, min(3, X), replace=False), "three_points"),
            ]:
                if len(supp):
                    cases.append((int(j), make_dist(X, supp, rng), label))
            # A tail just inside the stopping threshold probes endpoint sensitivity.
            p = int(neg[0])
            pos = np.flatnonzero(A[j] == 1)
            q = int(pos[0])
            den = TAU.denominator * 8
            if den >= 2**62:
                raise RuntimeError("distribution denominator too large")
            w = np.zeros(X, dtype=np.int64)
            tail = INNER / 4 + TAU / 2
            w[p] = int(tail * den)
            w[q] = den - w[p]
            cases.append((int(j), w, "threshold_tail"))
        # Random restarts followed by coordinate ascent on either error or rounds.
        for restart in range(args.restarts):
            j = int(rng.choice(eligible))
            supp = rng.choice(X, min(20, X), replace=False)
            forced = np.flatnonzero(A[j] == -1)
            supp[: min(len(forced), len(supp))] = forced[: min(len(forced), len(supp))]
            supp = np.unique(supp)
            ...
            def score(z):
                return (
                    (z["error"], z["rounds"]) if criterion == "error" else (z["rounds"], z["error"])
                )

            base = learn(A, sampler, j, w, seed + restart, "endpoint", search_proper, False)
            for step in range(args.steps):
                cand = w.copy()
                support = np.flatnonzero(cand)
                if rng.random() < 0.5 and len(support) > 1:
                    aa, bb = rng.choice(support, 2, replace=False)
                    transfer = max(1, int(cand[aa] * rng.uniform(0.1, 0.8)))
                    cand[aa] -= transfer
                    cand[bb] += transfer
                else:
                    aa = int(rng.choice(support))
                    bb = int(rng.integers(X))
                    cand[bb] += cand[aa]
                    cand[aa] = 0
                ...
                if score(z) > score(base):
                    ...
                    w = cand
            cases.append(
                (j, w, "searched_" + ("proper_" if search_proper else "improper_") + criterion)
            )
        start = len(all_runs)
        for ci, (j, w, label) in enumerate(cases):
            for proper in [False, True]:
                for mode in ["exact", "endpoint"]:
                    z = learn(A, sampler, j, w, seed + 10000 + ci, mode, proper, True)
                    z.update(N=N, distribution=label)
                    raw.write(json.dumps(z, separators=(",", ":")) + "\n")
                    raw.flush()
                    all_runs.append(z)
        sampler.save(OUT / f"cap_certificates_N{N}.json")
        sampler.certstream.close()
        for proper in [False, True]:
            for mode in ["exact", "endpoint"]:
                ...
                item = {
                    ...
                    "oracle": mode,
                    "runs": len(rr),
                    "max_error": max(z["error"] for z in rr),
                    "max_rounds": max(z["rounds"] for z in rr),
                    "max_queries": max(z["queries"] for z in rr),
                    "peel_fraction": sum(z["peels"] for z in rr) / max(1, total),
                    "elimination_fraction": sum(z["eliminations"] for z in rr) / max(1, total),
                    "rejection_fraction": sum(z["rejects"] for z in rr) / max(1, total),
                    "cap_certificates": len(sampler.certs),
                    "max_isotropy_residual": sampler.max_resid,
                }
                summary.append(item)
                print(json.dumps(item), flush=True)
        print(
            f"N={N} completed; elapsed {time.monotonic() - tic:.1f}s, transform/catalog time {sampler.seconds:.1f}s",
            flush=True,
        )
    raw.close()
        ...
        json.dumps(
            {
                ...
                "coordinate_steps": args.steps,
                "notes": "Finite point-centered and Gaussian cap pool; exact pointwise certificate. Numerical radial isotropy, not an exact real transform. Public-transcript greedy endpoint adversary; no private row side/color is read during marginal answers. Not a worst-policy certificate.",
            },
            indent=2,
        )
    )
...
if __name__ == "__main__":
    ...
    ap.add_argument("--N", nargs="+", type=int, default=list(range(2, 9)))
