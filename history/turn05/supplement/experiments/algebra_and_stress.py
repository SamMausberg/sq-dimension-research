...
import itertools,json,csv,math,random,hashlib
from seeded_fast import SeedClass,Oracle,learn,KEY,SEED
 ...
 for T in range(4):
  ...
  for labs in itertools.product([-1,1],repeat=N):
   for sample in itertools.product(range(N),repeat=T):
    seen=set(sample)
    err+=F(sum(labs[i]==-1 for i in range(N) if i not in seen),N);count+=1
  err/=count;exact=F(1,2)*F(N-1,N)**T
...
rows=[]
for k in range(2,7):
 K=2*k**3;I=2*k**4-(k*(k+1)//2)**2;D=math.floor(k/(24*math.log2(k)))
 brute=sum(1 for a in range(1,k+1) for b in range(1,2*k*k+1) for x in range(1,k+1) if a*x+b<=2*k*k)
 assert I==brute and I>=k**4 and D==0
 rows.append(dict(k=k,rows=K,points=K,incidence_bits=I,D=D,half_k4=str(F(k**4,2)),low_rank_event='empty: strict rank zero impossible'))
with (OUT/'subgrid_counts.csv').open('w') as f:
 w=csv.DictWriter(f,fieldnames=rows[0]);w.writeheader();w.writerows(rows)
# Stress a large-star branch beyond the tabulated N<=32 sweep.
N=4096;cls=SeedClass(N);p=(1,N+1)
negative=[a for a in range(1,N+1) if cls.label(a,N+1-a,p)==-1]
assert len(negative)>=N/4
star=[]
for seed in range(20):
 a=negative[seed];target=(a,N+1-a);points=[p,(1,2*N*N)];weights=[700,300]
 ...
 star.append(dict(seed=seed,target=target,output=out,queries=len(O.trace),error=str(loss),trace=O.trace))
(OUT/'large_star.json').write_text(json.dumps(dict(N=N,key_hex=KEY.hex(),negative_rows=len(negative),star_size=N,runs=star),separators=(',',':')))
(OUT/'unseen_exact.json').write_text(json.dumps(unseen,indent=2))
(OUT/'audit.json').write_text(json.dumps(dict(endpoint_algebra_checks=checks,unseen_cases=len(unseen),subgrids=len(rows),large_star_runs=len(star),large_star_negative_count=len(negative),all_passed=True),indent=2))
