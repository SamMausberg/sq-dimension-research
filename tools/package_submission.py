...
def main() -> None:
    ...
    parser.add_argument('--skip-render-compare', action='store_true',
    ...
    selected = sorted(p for p in paper.rglob('*') if p.is_file()
    ...
    report_path = ROOT / 'audits/current/clean_package.json'
    with tempfile.TemporaryDirectory(prefix='sq-dc-arxiv-') as temporary:
        ...
        subprocess.run([sys.executable, str(ROOT / 'tools/build_paper.py'),
                        '--directory', str(clean), '--report', str(report_path)], check=True)
        report = json.loads(report_path.read_text())
        report.update(archive_sha256=sha256(archive), source_files=len(selected),
                      entry_point='paper.tex', clean_extraction=True)
        if not args.skip_render_compare:
            try:
                import fitz
            except ImportError as exc:
                raise SystemExit('Install requirements-dev.txt or use --skip-render-compare.') from exc
