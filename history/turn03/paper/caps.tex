...
The linear program in Lemma~10 has exponentially many possible rectangles. A fixed-dimensional template makes a different sampler available. Exact radial isotropy is used to state its clean geometric constant; the algorithmic corollary uses a rational approximation and a finite net. Exact real transformations and finite-bit computation are not identified.
...
\begin{equation}\label{eq:capcoverage}
 ...
 [u_1\ge\sqrt{1-1/(2d)}].
\end{equation}
Here $c_1=1/2$. Thus Theorem~9 applies with $\rho=8d/c_d$ in the ideal real-arithmetic model.
...
For finite-bit computation, Appendix~\ref{app:convex} gives a deterministic rational transform with normalized covariance at least $3I_j/(4j)$. The preceding second-moment argument then gives total side mass at least $1/(4j)$, selected-side mass at least $1/(8j)$, and split coverage at least $1/(16j)$ whenever a point belongs to the cap. There is no claim that an exact irrational optimum is computed.
...
\] The row-side tests use the analogous strict squared comparison. Consequently no exact sampling of a continuous direction is required. Theorem~9 with $r_d$ proves the randomized finite-bit bound.

For determinism, enumerate the at most $2J_j$ outcomes and query each $D(S)$, using the same tolerance and stopping check as Theorem~9 with $r_d$. Choose the outcome maximizing $b\widehat s$, where $b=\nu(T)$. Coverage implies that some outcome has $bs\ge r_du$. The chosen one has
\[
