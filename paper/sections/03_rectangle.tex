...
The same theorem gives $m=O(\rho^2\log(e|X|)+\rho\log(1/\epsilon))$: the rectangle condition bounds VC dimension by $\rho/2$, and the finite trace count bounds $\log K$. The proof is in \cref{proof:main}. \Cref{fig:rectify} depicts the two updates.
