...
def ambient_encode(exponent: int, a: int, b: int, x: int, y: int) -> str:
    ...
    return "".join(
        format(z - 1, f"0{w}b")
        for z, w in zip((a, b, x, y), (exponent, 2 * exponent + 1, exponent, 2 * exponent + 1))
    )
...
def run() -> dict:
    ...
    for atoms in itertools.product(range(4), repeat=4):
        ...
        if not total:
            continue
        weights = [F(a, total) for a in atoms]
        beta = F(1)
        z = F(1, 512)
        for eb in [-1, 1]:
            bh = beta + eb * z
            for answers in itertools.product([-1, 1], repeat=3):
                ...
                while high - low > 1:
                    mid = (low + high) // 2
                    value = sum(weights[: mid + 1]) + answers[i] * z
                    i += 1
                    if value >= bh / 2:
                        high = mid
                    else:
    ...
    for t in range(1, 1001):
        power = 1 << (t - 1).bit_length()
        mult = Counter(i % t for i in range(power))
        ...
        for a in range(1, k + 1):
            for intercept in range(1, 2 * k * k + 1):
                for x in range(1, k + 1):
                    ...
                    subbit = (
                        hashlib.sha256(ambient_encode(b, a, intercept, x, y).encode()).digest()[0]
                        & 1
                    )
    ...
    for k in list(range(2, 257)) + [512, 1024, 2048]:
        ...
        if k <= 6 or D and not any(q["D"] > 0 for q in subgrid):
            subgrid.append(dict(k=k, K=K, I=I, D=D))
    raw["subgrid"] = subgrid
        ...
        for a in range(1, N + 1):
            for b in range(1, N * N + 1):
                for x in range(1, N + 1):
                    bits[a, b, x] = rng.getrandbits(1) * 2 - 1
    ...
    for n in [1, 3, 5, 11, 31, 101]:
        for eps in [F(1, 5), F(1, 10), F(1, 100)]:
            gamma = F(1, n)
            tau = eps * gamma / 8
            T = math.ceil(100 / (eps * eps * gamma * gamma))
            a = 1 / math.sqrt(T)
