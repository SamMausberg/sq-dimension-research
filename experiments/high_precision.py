...
def solve(V):
    ...
    def calc(t, hessian=False):
        ...
        for i in range(k - 1):
            for j in range(k - 1):
                H[i, j] = (lev[i] if i == j else 0) - w[i] * w[j] * C[i, j] ** 2
        return f, g, H, S
    ...
    if k == d + 1:
        ...
        for omitted in range(k):
            ...
            if determinant == 0:
                raise RuntimeError("Input rows not in general position")
            logs.append(2 * mp.log(abs(determinant)))
        t = mp.matrix([a - logs[-1] for a in logs])
    for it in range(400):
        f, g, H, S = calc(t, True)
        if max(abs(z) for z in g) < mp.mpf("1e-24"):
            break
        ...
        if maxstep > 10:
            step = step * (10 / maxstep)
        if sum(g[i] * step[i] for i in range(k - 1)) >= 0:
            step = -g
        dg = sum(g[i] * step[i] for i in range(k - 1))
        scale = mp.mpf(1)
        for _ in range(150):
            trial = mp.matrix(t)
            for i in range(k - 1):
                trial[i] += scale * step[i]
            nf, _, _ = calc(trial)
            if nf <= f + mp.mpf("0.01") * scale * dg:
                break
            scale /= 2
        else:
            raise RuntimeError("High-precision line search failed")
        t = trial
    f, g, S = calc(t)
    ev, Q = mp.eigsy(S)
    ...
    if err > 1e-10:
        raise RuntimeError(f"High-precision residual {err}")
