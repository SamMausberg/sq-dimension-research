...
C=SeedClass(N,KEY);K=len(points)
...
for offset,label,template in [(0,r'Template $F$',True),(22,r'Keyed flip $A_s$',False)]:
    for i,(a,b) in enumerate(lines):
        for j,p in enumerate(points):
            ...
            if negative:s.append(f'\\fill ({offset+j},{K-i-1}) rectangle ++(1,1);')
    s.append(f'\\draw[step=1,thin] ({offset},0) grid ({offset+K},{K});')
    s.append(f'\\node[above] at ({offset+K/2},17) {{{label}}};')
s += [r'\end{tikzpicture}',r'\caption{The $N=2$ template and the keyed flip used in the CPU tests. Black cells are negative and white cells positive. Rows index lines and columns index points. Only black template cells may change. This small example illustrates the construction; the counting lower bound is asymptotic.}',r'\label{fig:template}',r'\end{figure}']
