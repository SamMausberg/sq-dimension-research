...
for p in (2, 3, 5, 7, 11):
    ...
    P = np.array(
        [[int((a * x + b - y) % p == 0) for x, y in points] for a, b in lines], dtype=np.int64
    )
    K = p * p
    ...
    lam = F(p, 2) * (1 + F(1, (p - 1) ** 2))
    ratio = F(K) / lam
    assert ratio >= p
    rows.append(
        {"p": p, "rows": K, "lambda": str(lam), "R": str(ratio), "integer_gram_verified": True}
    )
prob = []
for p in (1031, 2053, 4099, 65537):
    ...
    conservative = float(alpha) * p / (t * t) - 0.25
