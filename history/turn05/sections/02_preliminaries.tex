...
$|v-\E_Dq(x,h(x))|\le\tau$ \citep{Kearns98}. An oracle policy may choose its answers using the entire public query-answer history. We use the strongest adaptive-policy reading of arbitrary valid responses. A randomized learner must satisfy
...
The literal raw-score loss $\Pr[h(x)\langle w,\phi(x)\rangle<0]$ would assign zero loss to $w=0$ for every class. That formula therefore gives a vacuous probabilistic dimension unless the prediction or tie rule is specified. Equations \eqref{eq:prob-definition}--\eqref{eq:approx-definition} use Boolean predictions. \Cref{lem:ties} establishes the same asymptotic lower bounds for the opposite tie rule and for a convention counting zero scores as errors. These are the nondegenerate probabilistic formulations refuted here.
...
For any finite family of predictions obtained from a $d$-dimensional embedding with the convention $\sign(0)=+1$ or $-1$, the same Boolean prediction matrix has a strict realization in dimension at most $d+1$. The probabilistic lower bounds in \cref{thm:prob,lem:exact-prob} remain valid under either tie rule or when all zero scores count as errors.
\end{lemma}
